...
{
	printf("TASK:%d\t%lu\t%llu\t%llu\t%llu\t%llu\n",
	task->task_id,
	task->priority,
	task->exec,
	task->period,
...
void assign_priorities(struct task *tasks, int ntasks)
{
	int i, j, min, swap;
	unsigned long long to_sort[ntasks];

	for(i=0;i<(ntasks);i++)
	{
		to_sort[i] = tasks[i].period;
	}
	for(i=0;i<(ntasks);i++)
	{
		min = i;
...
///////////////////////////////////////////////////////////////
	...
	assign_priorities(tasks, ntasks);
	...
	for(i=0;i<ntasks;i++){
		...
		sprintf(arg[7],"%lu",tasks[i].priority);
		pid_casio_tasks[i]=fork();
		if(pid_casio_tasks[i]==0){
			execl("./casio_task","casio_task",arg[0],arg[1],arg[2],arg[3],arg[4],arg[5],arg[6],arg[7],NULL);
